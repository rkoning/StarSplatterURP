...
public class MeshGenerator : MonoBehaviour {
    ...
    public void Run(TextureMinMax textureMinMax) {
    ...
    public void UpdateChunkMesh (Chunk chunk) {
        ...
        for (int i = 0; i < numTris; i++) {
            for (int j = 0; j < 3; j++) {
                ...
                if (textureMinMax != null) {
                    textureMinMax.SetValue(tris[i][j].sqrMagnitude);
                }
            }
        }
    ...
    void InitChunks () {
        CreateChunkHolder ();
        chunks = new List<Chunk> ();
        List<Chunk> oldChunks = new List<Chunk> (chunkHolder.GetComponentsInChildren<Chunk>());
    ...
    Chunk CreateChunk (Vector3Int coord) {
        ...
        chunk.layer = chunk.transform.parent.gameObject.layer;
        chunk.tag = "Surface";
